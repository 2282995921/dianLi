...
export default {
  ...
  watch: {
    search_vla: {
      handler: function() {
        this.$get(keywordSearch, {lookForQuestion:this.search_vla}).then(res => {
  ...
  methods: {
    ...
    getConent() {
      axios.get('https://www.mocky.io/v2/5185415ba171ea3a00704eed',{data:this.$route.query.id}).then((data) => {
        console.log(data)
      }).catch((error) => {
        console.log(error);
      });
